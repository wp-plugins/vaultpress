...
<?php
	...
	function parse_request( $wp ) {
		...
		if ( !$this->validate_api_signature() ) {
			...
			die( 'invalid api call signature [' . base64_encode( $__vp_validate_error ) . ']' );
		}
	...
	function validate_api_signature() {
		...
		if ( !empty( $_POST['signature'] ) )
			...
			$__vp_validate_error = 'no_signature';
			return false;
		}
		...
		if ( !is_array( $sig ) || count( $sig ) != 2 || !$sig[0] || !$sig[1] ) {
			$__vp_validate_error = "invalid_signature_format";
			return false;
		}
		...
		$__vp_validate_error = "invalid_signed_data";
		return false;
	}
	...
	function validate_ip_address( $rxs ) {
		global $__vp_validate_error;
		if ( empty( $rxs ) ) {
			$__vp_validate_error = 'empty_vp_ip_range';
			return false;
		}
		...
		if ( empty( $remote_ips ) ) {
			$__vp_validate_error = 'no_remote_addr.' . (int)$this->get_option( 'allow_forwarded_for'); // shouldn't happen
			return false;
		}
		...
		if ( function_exists( 'filter_var' ) ) {
			foreach ( $remote_ips as $remote_ip ) {
				if ( !filter_var( $remote_ip, FILTER_VALIDATE_IP ) ) {
					$__vp_validate_error = 'remote_addr_fail.' . $remote_ip;
					return false;
				}
			}
			return true;
			...
			foreach ( $remote_ips as $remote_ip ) {
				$remote_ip = preg_replace( '#^::(ffff:)?#', '', $remote_ip );
